...
model_file = "edsr_20m_v0"
save_path = "../Dataset/" + model_file + "_SR/"

model_dict = {
    "edsr": models.edsr_20m,
...
for path in tqdm(os.listdir("../Dataset/Unscaled/")):
    if path[path.find("_")+1:path.find(".")] in ["4", "5", "6", "7", "8", "9"]: 
        ...
        im = im / 255.0
        input_im = trans(im).to(device).float()
        sr = 255.0 * trained_model(input_im).detach().cpu().numpy().reshape((3, 200, 200))[0, :, :]
